...
push!(elec_angle, rem2pi(pole_pairs * input[1], RoundNearest))
...
for i in 1 : samplecount
    if (timebase[i] - last(daq_timebase)) < daq_period
        ...
        push!(elec_angle, rem2pi(pole_pairs * input[i], RoundNearest))
    end
end
...
# Approach II
# input_filter = zeros(samplecount)
# delay = 1000e-6
# ndelay::Int = delay * daq_freq
# for i in ndelay+1 : samplecount
#     input_filter[i] = input[i-ndelay]
# end
